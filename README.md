...
- 배치 실행 이력 : 배치 실행 결과 및 오류 메시지 기간별 조회
...
## ✅️ 기술적 고려사항
- OMS 연동을 위한 JSON payload 구조 설계 (baseDate, clientId, SKU별 alerts 포함)
- OMS 미설정 고객사 예외 분기 및 상황별 로그 레벨 구분(INFO/WARN/ERROR) 적용
- OMS 발송 배치 실행 전 분석 배치 정상 완료 여부를 BATCH_LOG 기반으로 검증하여 데이터 정합성 확보
- 발송 성공/실패 이력을 DB에 저장하여 재발송 및 장애 추적 지원
...
```mermaid
flowchart TD
    A([02:30 재고 분석 배치]) --> B[재고/출고 이력 조회]
    B --> C[일평균 출고량 계산 - 7일/28일]
    C --> D{잔여일수}
    D -->|≤ 7일| E[DANGER]
    D -->|≤ 14일| F[WARNING]
    D -->|> 14일| G[SAFE]
    ...
    K([05:00 OMS 발송 배치]) --> L{분석 배치 SUCCESS?}
    L -->|NO| M[발송 중단]
    L -->|YES| N[고객사 목록 조회]
    N --> O{OMS URL?}
    O -->|없음| P[(SKIP)]
    O -->|있음| Q{WARNING/DANGER?}
    Q -->|없음| R[발송 생략]
    Q -->|있음| S[JSON payload 구성]
    S --> T[HTTP POST]
    T -->|성공| U[(SUCCESS)]
    T -->|실패| V[(FAIL)]
```
